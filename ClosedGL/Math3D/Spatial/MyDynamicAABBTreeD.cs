...
namespace VRageMath
{
    ...
    public class MyDynamicAABBTreeD 
    {
        ...
        //public delegate double RayCastCallbackInternal(ref MyRayCastInput input, int userData);
        ...
        internal class DynamicTreeNode
        {
            ...
            {
                return Child1 == MyDynamicAABBTreeD.NullNode;
            }
        }
        ...
        public int AddProxy(ref BoundingBoxD aabb, object userData, uint userFlags, bool rebalance = true)
        {
            using (m_rwLock.AcquireExclusiveUsing())
            {
                ...
                m_leafElementCache[proxyId] = m_nodes[proxyId];
        ...
        public bool MoveProxy(int proxyId, ref BoundingBoxD aabb, Vector3D displacement)
        {
            using (m_rwLock.AcquireExclusiveUsing())
            {
                ...
                Vector3D r = m_extension;
                b.Min -= r;
                b.Max += r;

                // Predict BoundingBox displacement.
                Vector3D d = m_aabbMultiplier * displacement;
        ...
        private void InsertLeaf(int leaf, bool rebalance)
        {
            ...
            BoundingBoxD leafAABB = m_nodes[leaf].Aabb;
            ...
            while (m_nodes[index].IsLeaf() == false)
            {
                ...
                if (rebalance)
                {
                    double area = m_nodes[index].Aabb.Perimeter;

                    // Combined area
                    BoundingBoxD combinedAABB = BoundingBoxD.CreateMerged(m_nodes[index].Aabb, leafAABB);
                    ...
                    {
                        BoundingBoxD aabb;
                        BoundingBoxD.CreateMerged(ref leafAABB, ref m_nodes[child1].Aabb, out aabb);
                    ...
                    else
                    {
                        BoundingBoxD aabb;
                        BoundingBoxD.CreateMerged(ref leafAABB, ref m_nodes[child1].Aabb, out aabb);
                        double oldArea = m_nodes[child1].Aabb.Perimeter;
                    ...
                    {
                        BoundingBoxD aabb;
                        BoundingBoxD.CreateMerged(ref leafAABB, ref m_nodes[child2].Aabb, out aabb);
                    ...
                    else
                    {
                        BoundingBoxD aabb;
                        BoundingBoxD.CreateMerged(ref leafAABB, ref m_nodes[child2].Aabb, out aabb);
                        double oldArea = m_nodes[child2].Aabb.Perimeter;
                        double newArea = aabb.Perimeter;
                ...
                else
                {
                    ...
                    BoundingBoxD aabb1;
                    BoundingBoxD aabb2;
                    BoundingBoxD.CreateMerged(ref leafAABB, ref m_nodes[child1].Aabb, out aabb1);
                    BoundingBoxD.CreateMerged(ref leafAABB, ref m_nodes[child2].Aabb, out aabb2);
                    double norm1 = (m_nodes[child1].Height + 1) * aabb1.Perimeter;
                    double norm2 = (m_nodes[child2].Height + 1) * aabb2.Perimeter;
            ...
            m_nodes[newParent].Aabb = BoundingBoxD.CreateMerged(leafAABB, m_nodes[sibling].Aabb);
            ...
            while (index != NullNode)
            {
                ...
                BoundingBoxD.CreateMerged(ref m_nodes[child1].Aabb, ref m_nodes[child2].Aabb, out m_nodes[index].Aabb);
        ...
        private void RemoveLeaf(int leaf)
        {
            ...
            if (grandParent != NullNode)
            {
                ...
                while (index != NullNode)
                {
                    ...
                    m_nodes[index].Aabb = BoundingBoxD.CreateMerged(m_nodes[child1].Aabb, m_nodes[child2].Aabb);
                    m_nodes[index].Height = 1 + Math.Max(m_nodes[child1].Height, m_nodes[child2].Height);
        ...
        public void OverlapAllFrustum<T>(ref BoundingFrustumD frustum, List<T> elementsList, List<bool> isInsideList, uint requiredFlags, bool clear = true)
        {
            if (clear)
            {
                elementsList.Clear();
        ...
        {
            OverlapAllLineSegment<T>(ref line, elementsList, 0, clear);
        }
        ...
        public void GetAll<T>(List<T> elementsList, bool clear, List<BoundingBoxD> boxsList = null)
        {
            ...
            using (m_rwLock.AcquireSharedUsing())
            {
                foreach (var node in m_leafElementCache)
                {
